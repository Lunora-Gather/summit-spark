...
if (!js.includes("if (settingsVisible || entryGate?.classList.contains(\"hidden\")) return;") || !js.includes("guestEntryButton?.focus({ preventScroll: true });") || !css.includes(".stage:has(.start-panel.entry-pending) + .portrait-brief") || !browserSmoke.includes("320px first-run entry should stay focused, touch-safe and free of background room coaching")) errors.push("the asynchronous first-run chooser must focus Guest and suppress portrait room coaching through safe-area phone widths");
...
if (!js.includes("const SAVE_ARCHIVE_MAX_CHARS = 1000000;") || !browserSmoke.includes("runLargeCloudArchiveSmoke") || !browserSmoke.includes("a full ten-room route archive above the legacy 240k cap must upload and remain self-importable")) errors.push("complete ten-room route archives must remain below a conservative 1MB client guard and round-trip through cloud upload and local import");
