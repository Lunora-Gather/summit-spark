...
if (!indexHtml.includes('id="settingsBackdrop" aria-hidden="true"') || !css.includes(".settings-backdrop") || !js.includes('settingsBackdrop?.addEventListener("click", closeSettingsFromBackdrop)') || !js.includes("function pointHitsVisibleElement") || !browserSmoke.includes("outside start-button region closes without click-through") || !browserSmoke.includes("real outside training-button pointer switches panels") || !browserSmoke.includes("real outside account-button pointer switches panels")) errors.push("settings must use a real non-inert backdrop that waits for click completion, blocks click-through and switches intended outside actions with a real pointer");
