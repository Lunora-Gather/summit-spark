...
- `20260908-p310`：首次自动聚焦游客按钮时不再显示焦点圈；开始键盘导航后显示按钮内侧深青细线，移除原外扩双圈与描边阴影。新增首屏无圈及 Tab 导航细线回归；尺寸、配色、入口选择和玩法不变。

- `20260908-p309`：主菜单按实际游戏画面宽度缩放，桌面按钮在 300–400px 之间、标题在 48–76px 之间自适应，手机仍保留紧凑布局。设置宽度收至 440px，换为细分隔线和统一线性图标列表，规范标题/关闭键/控件对齐；五个滑块显示即时数值并同步无障碍读数。新增跨窗口比例、设置列表及滑块输入回归；地图、角色、页面外框与账号流程不变。

- `20260908-p308`：按“月夜山景里的攀登入口”重新组织 UI。首次选择与主菜单复用无外框启动组件、暖金标题和实色按钮，去除独立游玩方式白卡与 S 图标；整理入口/账号历史样式，保留网页浅色环境、山景、地图和原小人固定发色。
- 账号表单改用对称留白、更清晰的标题/字段/说明、按内容收缩的移动面板；移除空分栏横线与常驻滚动槽。错误/成功颜色不再被普通文字样式覆盖，密码页不再误称首次密码登录可自动建号。
- 实际截图检查桌面、390px 竖屏、320px 短屏与700px 横屏；浏览器回归新增启动样式同源、表单对齐、底部空白、状态颜色与密码文案断言，继续覆盖启动失败、软键盘、安全区和登录状态机。设计约定记录于 `docs/UI_DESIGN.md`。

## 2026-09-07

- 完成 `20260907-p307` 的账号聚焦卡片收口。移除账号表单内部重复的浅色分组背景，避免出现“卡片套卡片”的错觉；字段、验证码、说明和状态现在直接落在统一外层纸面上，并加入内层背景/阴影回归。账号流程、地图、角色外观和固定发色不变。

- 完成 `20260907-p306` 的首屏入口视觉收口。选择游客/邮箱登录与首屏账号表单现在共用同一套纸面卡片语言、宽度、圆角、阴影、标题层级和安全触控尺寸；修正账号表单在小屏上的 42px 控件回退，并增加入口卡片与账号卡片的几何/表面一致性回归。核心地图、小人、固定发色、账号流程与运行中设置布局不变。

- 完成 `20260907-p305` 的首屏账号专注态收口。首屏点击“云存档”后，启动菜单不再以半透明残影留在背景，账号面板在桌面端居中显示；运行中账号入口保持原右侧布局，移动端继续使用安全区铺满面板。新增启动账号几何与层级回归。

## 2026-09-07

- 完成 `20260907-p304` 的首屏 UI 细节收口。启动选择层加入克制的中心聚焦雾层，避免标题和主按钮被后方平台切断；账号入口的标签、说明和输入字号提升到稳定可读级别，并新增真实浏览器回归确认样式确实生效。地图、物理、角色外观、固定发色、页面层级和原有视觉基调不变。

## 2026-09-07

- 完成 `20260907-p303` 的竖屏弹层定位收口。运行中打开设置时，舞台上移不再把设置面板一起带离视口；320×480、390×700 等窄屏下，面板与关闭按钮完整可见且保持触控尺寸。新增真实视口几何回归，游戏内容、角色外观、固定发色和页面基调不变。

## 2026-08-16

- 完成 `20260816-p302` 的多指触控输入收口。每个触控方向/动作按钮现在只绑定第一根活动手指，后续手指的 pointerup、pointercancel、pointerleave 或失配事件不会提前释放原按键；失去 Pointer Capture 仍会按原活动指针及时释放。新增真实浏览器双指回归，键盘、手柄、地图、物理、角色外观、固定发色和页面基调不变。

## 2026-08-16

- 完成 `20260816-p301` 的训练状态收口。Drill 中向左回退离开当前房间时，现在会清理旧房间的 Drill，并取消与该 Drill 绑定的 Route/Feel 状态，避免上一房残留训练 HUD、目标文案或错误续练；跨房挑战、已记录的训练历史、普通路线和微光规则不变。新增真实浏览器回归覆盖回退后的普通游玩态。

## 2026-08-16

- 完成 `20260816-p300` 的回退跨房状态收口。向左回到上一房时会重新武装当前房间的 Clean 尝试，但保留整局失误账本；已经跨过前一房边界结算的微光会作为合法营灯快照继续保留，后续死亡不会错误抹除；跨章节回退也恢复既有章节过渡演出与声音边界。新增真实浏览器回归覆盖同章/跨章回退、Clean 判定和已结算微光，角色造型、固定发色、地图、物理、页面基调和奖励规则不变。

## 2026-08-16

- 完成 `20260816-p299` 的 R3 回归探针收口。补给点测试现在依据实际支撑平台边界并保留 grounded 判定，允许低帧率/合成器时序带来的少量像素误差，不再把仍在安全平台上的合法落点误报为失败；游戏地图、碰撞与补给规则不变。

## 2026-08-16

- 完成 `20260816-p298` 的手柄 API 异常隔离。浏览器在后台、权限切换或设备热插拔期间若暂时拒绝 `getGamepads()`，当前帧会按“未连接”处理并在下一帧恢复，不再把设备 API 异常升级成主循环错误。键位映射、死区、手柄动作和页面 UI 不变，并新增真实浏览器异常轮询回归。

## 2026-08-16

- 完成 `20260816-p297` 的长期奖励结算顺序修复。完整路线结算现在先写入本轮新的整局 Flow 与全微光最佳值，再立即重算长期挑战；首次达成不会再延迟到下一次刷新或打开训练页才显示。PB、辅助隔离、收集回滚、房间内容和角色外观不变。

## 2026-08-16

- 完成 `20260816-p296` 的音频上下文生命周期收口。浏览器或系统关闭旧 AudioContext 后，环境音句柄、音效冷却时间和旧音频图会一起清理；新上下文会从干净时间基准恢复，单帧音频异常也不会把主循环拖进假死。新增“不可用 → 恢复 → 替换上下文试听”浏览器回归，正式玩法、声音设计、房间、奖励、角色外观和固定头发颜色不变。

## 2026-08-16

- 完成 `20260816-p295` 的音频可用性反馈收口。声音试听现在会先确认 AudioContext 已创建并成功恢复到运行态；浏览器拒绝音频权限、设备无 Web Audio 或上下文已关闭时，状态会明确提示“声音暂不可用”，不再把静音误报成试听成功。新增音频不可用/恢复双路径回归。

- 完成 `20260816-p294` 的首启上下文恢复收口。若页面还没开始游戏就遇到 Canvas 丢失，恢复后现在会清掉遗留的焦点暂停状态；开始攀登、挑战、练习和调试跳房在上下文仍不可用时也会安全等待，不会生成“已进入但永远暂停”的首局。新增首启前 `contextlost → resize → contextrestored → 开始 → 移动` 回归。

- 完成 `20260816-p293` 的 Canvas 旋转竞态收口。绘制上下文丢失期间，设备旋转或窗口 `resize` 不再尝试写入失效上下文；恢复事件会显式强制重建缓冲并重新接回帧链，避免“恢复后又被 resize 异常打断”。浏览器回归新增丢失→resize→恢复序列。

- 完成 `20260816-p292` 的启动失败恢复收口。任一核心模块未能加载或初始化时，启动页不再留下看似可点却尚未绑定的菜单按钮；运行时会进入明确的错误状态、隐藏死操作、解释网络/缓存原因，并提供不会清除本地存档的“重新加载游戏”入口。新增本地专用故障注入与真实浏览器回归，验证失败页面可读、可操作且不会误报应用已就绪。

- 完成 `20260816-p291` 的 Canvas 上下文恢复收口。浏览器或移动设备因内存压力触发 `contextlost` 时，游戏会主动阻止默认销毁、释放输入并显示恢复状态，避免进入“画面存在但按键无效”的假死；`contextrestored` 后会重建画布缓冲、重置帧时钟并恢复移动。新增真实浏览器合成丢失/恢复事件回归，确认恢复后仍可继续前进。

- 完成 `20260816-p290` 的设置关闭幂等收口。设置面板现在把重复的 pointerup、touchend 与 click 关闭事件视为同一次关闭：状态只切换一次、输入只释放一次，并稳定把焦点还给打开设置的入口；新增真实浏览器回归确认双击关闭不会留下隐藏面板焦点或暂停状态残留。

- 完成 `20260816-p289` 的存档可见性收口。运行中任何关键本地存档写入（设置、PB、Flow、房间路线、Focus、档案或导入事务）失败时，都会复用现有存档健康提示，说明本地进度可能无法写入，同时不阻断当前游玩；新增浏览器回归主动注入配额失败并确认提示出现。

- 完成 `20260816-p288` 的触控中断恢复收口。触控按钮现在先登记按下再尝试 Pointer Capture，捕获 API 不可用或抛错时仍可降级操作；系统手势、浏览器栏、旋转或设备中断导致 `lostpointercapture` 时会立即释放方向/动作状态，避免移动或抓墙残留。真实移动端回归会保持右移、主动丢失捕获并确认角色及时停下。R1 Spark 键盘回归也改为页面内精确排程和有限重试，避免慢 CI 错过短动作窗；连续两轮完整浏览器门均通过。

- 完成 `20260816-p287` 的主循环错误边界收口。若渲染或更新链中出现单帧异常，`requestAnimationFrame` 不再永久中断；运行时会清理输入边沿与物理积累、尝试恢复当前检查点、显示可理解的暂停提示，并保留 `R` 重开后的可移动路径。新增真实浏览器注入测试，确认抛帧后仍能重开、移动且不产生未处理运行时错误。

- 完成 `20260816-p286` 的环境音生命周期收口。静音、音量归零、暂停、登顶或音频上下文暂不可用时，已排程的环境音声部会立即淡出并停止，不再让隐藏的振荡器继续占用长局资源；重新开启声音会从当前章节节拍重新接入。正式音效、章节声纹、键盘/手柄/触控输入与关卡内容不变。

- 完成 `20260816-p285` 的发布门稳定性收口。R10 顶点冲刺回归现在记录动作触发瞬间的真实速度，并使用开放上行测试通道避免慢帧把探针误判成尖刺死亡；正式键盘、手柄和触控输入路径不变。云端浏览器门禁与本地完整回归均通过。

- 完成 `20260816-p284` 的运行时结构自愈收口。有限状态哨兵现在会在任何房间实体集合、地图行、临时微光快照、角色头发轨迹或收集集合损坏后先于物理更新拦截异常，重建当前房间并恢复到可玩的检查点；浏览器回归主动注入结构损坏，确认恢复后仍能立即移动。该层保险不改变正常关卡、奖励、角色外观或手感。
