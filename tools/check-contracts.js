...
if (!indexHtml.includes('id="settingsBackdrop" aria-hidden="true"') || !css.includes(".settings-backdrop") || !js.includes('settingsBackdrop?.addEventListener("pointerdown", closeSettingsFromBackdrop)') || !js.includes("function pointHitsVisibleElement") || !browserSmoke.includes("real outside training-button pointer switches panels") || !browserSmoke.includes("real outside account-button pointer switches panels")) errors.push("settings must use a real non-inert backdrop so blank and underlying action regions dismiss or switch the panel with an actual pointer");
