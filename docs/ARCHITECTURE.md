...
`core/format.mjs`、`core/math.mjs`、`game/room-data.mjs`、`game/world-model.mjs`、`game/effect-budget.mjs`、`game/landmark-progress.mjs`、`game/audio-cues.mjs`、`game/lumen-progress.mjs`、`systems/storage.mjs`、`systems/input.mjs`、`training/state.mjs`、`training/replay.mjs` 与 `ui/presentation.mjs` 的首批低风险切片已完成；固定步长帧预算也已由 `core/math.mjs` 提供纯函数契约，主运行时只负责消费步数。p281–p287 的运行时收口继续把“重生可玩性”“响应式布局”“声音资源边界”和“帧循环错误边界”当成跨层契约：输入恢复、落点自愈、有限状态/集合恢复、静音/暂停时的环境音回收、单帧异常后的输入/调度清理只由主运行时编排，纯模块不读取 DOM；移动端位置、触控最小尺寸、长重启稳定性、非有限状态恢复、音频资源边界与抛帧后重开路径由同一套浏览器门禁验证。接下来的顺序是：
