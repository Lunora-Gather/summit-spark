...
`core/format.mjs`、`core/math.mjs`、`game/room-data.mjs`、`game/world-model.mjs`、`game/effect-budget.mjs`、`game/landmark-progress.mjs`、`game/audio-cues.mjs`、`game/lumen-progress.mjs`、`systems/storage.mjs`、`systems/input.mjs`、`training/state.mjs`、`training/replay.mjs` 与 `ui/presentation.mjs` 的首批低风险切片已完成；固定步长帧预算也已由 `core/math.mjs` 提供纯函数契约，主运行时只负责消费步数。p281–p290 的运行时收口继续把“重生可玩性”“响应式布局”“声音资源边界”“帧循环错误边界”“触控中断释放”“存档写入可见性”和“面板关闭幂等性”当成跨层契约：输入恢复、落点自愈、有限状态/集合恢复、静音/暂停时的环境音回收、单帧异常后的输入/调度清理、Pointer Capture 丢失后的数字输入释放、游玩中途存储不可写时的健康提示、重复关闭事件只执行一次并恢复入口焦点只由主运行时编排，纯模块不读取 DOM；移动端位置、触控最小尺寸、长重启稳定性、非有限状态恢复、音频资源边界、抛帧后重开路径、触控捕获中断、中途写入失败提示与设置面板双事件关闭由同一套浏览器门禁验证。接下来的顺序是：
