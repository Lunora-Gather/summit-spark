...
`core/format.mjs`、`core/math.mjs`、`game/room-data.mjs`、`game/world-model.mjs`、`game/effect-budget.mjs`、`game/landmark-progress.mjs`、`game/audio-cues.mjs`、`game/lumen-progress.mjs`、`systems/storage.mjs`、`training/state.mjs`、`training/replay.mjs` 与 `ui/presentation.mjs` 的首批低风险切片已完成；固定步长帧预算也已由 `core/math.mjs` 提供纯函数契约，主运行时只负责消费步数。p281–p302 的运行时收口继续把“重生可玩性”“响应式布局”“声音资源边界”“音频可用性反馈”“音频上下文生命周期”“长期奖励结算顺序”“手柄 API 异常恢复”“回归探针边界”“回退跨房状态”“房间训练中断”“帧循环错误边界”“触控中断释放”“多指触控归属”“存档写入可见性”“面板关闭幂等性”“Canvas 上下文恢复”“启动失败恢复”“旋转/resize 竞态”和“首启恢复”当成跨层契约：输入恢复、落点自愈、有限状态/集合恢复、回退时 Clean 尝试重新武装与已结算微光快照复用、跨章节回退的过渡演出恢复、离开 Drill 房间时 Route/Feel/训练 HUD 的清理、静音/暂停时的环境音回收、关闭/替换 AudioContext 时的旧时间基准与音频图清理、单帧音频异常后的安全丢弃、手柄轮询异常时按断开处理、单帧音频异常后的输入/调度清理、Pointer Capture 丢失后的数字输入释放、多指按钮只由第一根活动指针释放、游玩中途存储不可写时的健康提示、重复关闭事件只执行一次并恢复入口焦点、Canvas 丢失/恢复时的暂停与缓冲重建、模块加载失败时的死操作隔离与安全重载、上下文丢失期间 resize 跳过并在恢复时强制重建、首启前失焦状态清理与未恢复上下文时安全等待、试听失败时不宣称成功只由主运行时编排，纯模块不读取 DOM；移动端位置、触控最小尺寸、长重启稳定性、非有限状态恢复、音频资源边界、音频上下文替换、长期奖励结算、手柄轮询异常、回归探针边界、回退跨房状态、房间训练中断、抛帧后重开路径、触控捕获中断、中途写入失败提示、设置面板双事件关闭、Canvas 合成丢失/恢复、丢失期间旋转、音频可用/不可用/替换恢复路径、首启故障注入与启动故障注入、多指触控归属由同一套浏览器门禁验证。接下来的顺序是：
