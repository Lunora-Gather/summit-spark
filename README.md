...
| 动作 | 键盘 | 手柄 |
| --- | --- | --- |
| 移动/方向 | WASD 或方向键 | 左摇杆 / 十字键 |
| 跳跃 | Space / C / J | A |
| 冲刺 | X / K / Shift | B / X / RT |
| 抓墙 | Z / L / Ctrl | 肩键 / LT |
| 快速下落 | S / ↓ | 下方向 |
| 回声召回 | Q / Backspace | Y / Back |
| 快速重开 | R | — |
| 当前房重开 | T | — |
| 设置 | O / 右上角齿轮 | — |
| 练习 | P / 右上角练习按钮 | — |
...
当前运行时仍是单体脚本。短期不做高风险拆分；后续按纯函数、只读数据、存档、输入、训练、UI、物理、渲染的顺序迁移。完整边界见 [docs/ARCHITECTURE.md](docs/ARCHITECTURE.md)。
...
`npm run check` 覆盖语法、目录结构、CSP、Pages 发布边界、Appwrite 策略、GitHub Actions SHA 固定、地图、路线、训练状态、存档契约和本地 HTTP 安全边界。浏览器回归覆盖真实 UI、账号 mock、云冲突、1MB 存档、原子回滚、移动端、Canvas 和手柄死区。
...
P180 把真人完整通关到难度调优的证据链补完整：登顶后展开“更多复盘”即可复制一份本轮报告，包含构建号、总时间、四幕/十房用时、失误、Flow 与辅助状态；不含身份、设备名称、输入历史或路线坐标，复制失败时只在本地下载文本，不会上传。它不增加常驻 HUD，也不改变存档 schema。P179 的本轮统计与后台登顶兜底、P178 的幕末考试，以及 P177 恢复的旧轮廓和固定深蓝发色全部保持不变。
