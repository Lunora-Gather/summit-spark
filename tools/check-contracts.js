...
if (!browserSmoke.includes("runPracticeRecommendationIntegrationSmoke")
  ...
  || !browserSmoke.includes("three-room three-step plan")
...
if (!indexHtml.includes('id="deathCount" aria-label="失误次数">失 0</div>') || !js.includes('label: "零失误登顶"') || !js.includes('goal: "完整通关且失误数为 0"') || !js.includes('deathCountText.textContent = `失 ${deathCount}`') || !(js + trainingModule).includes('`最佳失误 ${') || !js.includes('<em>最佳失误</em>') || js.includes('deathCountText.textContent = `D ${deathCount}`') || js.includes('label: "零死亡登顶"')) errors.push("the visible counter and no-retry challenge should use the accurate mistake semantic while internal death save keys remain compatible");
