...
P181 修复了回声机制的触控断点：R9 的首枚锚点现在位于检查点旁的一格安全入口袋，首次激活会给出短暂的设备对应召回提示；触控端的“召”按钮只在含回声的房间占用原动作组空位，激活前休眠、可用时亮起、召回后显示冷却，不增加常驻 HUD。真实浏览器回归会执行一次完整的手机召回，并在启动阶段拦截运行时异常。手动重开后的失误计数也改为同步刷新。P180 的本轮报告、旧小人轮廓和固定深蓝头发保持不变。
