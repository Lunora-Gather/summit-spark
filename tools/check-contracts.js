...
if (!indexHtml.includes('id="accountAuthTabs" role="group" aria-label="登录方式"') || indexHtml.includes('role="tablist"') || indexHtml.includes('role="tab"') || !indexHtml.includes('data-auth-mode="code" aria-pressed="true"') || !indexHtml.includes('id="accountEmail" name="email"') || !indexHtml.includes('aria-describedby="accountStatus"') || !js.includes('button.setAttribute("aria-pressed", String(active));') || js.includes('button.setAttribute("aria-selected", String(active));') || !browserSmoke.includes("account login mode should expose segmented-button semantics and autofill-ready described fields")) errors.push("account login choices must use complete segmented-button semantics and expose autofill-ready, status-described fields");
if ((indexHtml.match(/class="settings-group-chevron" aria-hidden="true"/g) || []).length !== 9 || !indexHtml.includes('<summary aria-expanded="false">') || !css.includes(".settings-group summary > .settings-group-chevron") || !css.includes(".settings-group[open] summary::after") || !css.includes("content: none;") || !js.includes("function syncSettingsGroupDisclosure(group)") || !js.includes('setAttribute("aria-expanded", String(group.open))') || !browserSmoke.includes("settings disclosures should keep decorative chevrons out of accessible names and expose collapsed state")) errors.push("settings disclosures must expose synchronized expanded state while keeping every decorative chevron out of accessible names");
...
if (!js.includes("let cloudSaveDirty = false;") || !js.includes("let cloudSyncFlushRequested = false;") || !js.includes('setCloudStatus("有新进度等待同步", "待同步")') || !js.includes("function armCloudSyncTimer()") || !js.includes("if (cloudSyncBusy) {") || !browserSmoke.includes("a local change queued during a slow upload must receive a second upload with the latest archive")) errors.push("queued and in-flight local changes must remain visibly dirty and receive a lossless follow-up cloud upload");
