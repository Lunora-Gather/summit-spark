...
当前运行时主体仍是单体脚本；格式、安全文本、数学、只读房间内容、章节声纹、当前尝试地标进度、长局视觉效果预算、存档规则、设备无关的输入规则、训练状态与五类房间推荐、PB 路线动作语义，以及章节成绩、房间训练/进度摘要、练习优先级、四幕汇总和隐私受限本轮报告等 UI 展示模型已迁入独立模块。后续只迁移有直接消费者和回归契约的 UI 纯模型；物理与渲染等十房人工基线完成后再动。完整边界见 [docs/ARCHITECTURE.md](docs/ARCHITECTURE.md)。
