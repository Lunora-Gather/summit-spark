...
P179 给完整通关补上可用于继续打磨的本轮证据：游戏会在内存中累计十房用时与失误，登顶后的“更多复盘”以一张克制卡片比较四幕并指出最慢章节，诊断快照同时导出分房与分幕数据。它不增加常驻 HUD，也不改变存档 schema。P178 的幕末考试、登顶停留，以及 P177 恢复的旧轮廓与固定深蓝发色全部保持不变。
