...
`check-core-format.mjs`、`check-core-math.mjs`、`check-room-data.mjs` 和 `check-storage.mjs` 直接导入公开模块，固定纯函数输入输出、十房只读边界，以及存档设置迁移、读修复、档案/备份、上限/兼容和事务回滚；它们由 `npm run check` 自动执行。
