...
The next code PR may add the tiny compatibility helper near the current embedded constants in `summit-spark.js`.

That PR must still preserve the embedded source of truth and must not read from a new runtime data source.
...
P2 is ready for a small runtime compatibility seam PR. It is not ready for the actual runtime source switch yet.
